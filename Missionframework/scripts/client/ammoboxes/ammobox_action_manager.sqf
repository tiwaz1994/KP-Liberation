	...
	if ([player, 5] call F_fetchPermission) then {
...

		{
			...
			if (!(_next_box in _managed_boxes) && ( isNull  attachedTo _next_box )) then {
				...
				_b_action_id2 = _next_box addAction ["<t color='#FFFF00'>" + localize "STR_ACTION_STORE_CRATE" + "</t>",{[(_this select 0), (nearestObjects [player,KP_liberation_storage_buildings,200]) select 0,true] call F_crateToStorage;},"",-502,true,true,"","build_confirmed == 0 && (_this distance _target < 5) && (vehicle player == player)"];
